...
function wolfpack(model_path, pk_type) {
  
  if (!model_path) {
    return methods();
  }

  if (pk_type && !_.contains(['integer', 'string'], pk_type)) {
    throw new Error('WOLFPACK: Currently wolfpack only supports strings and integers as default type for id');
  }
  ...
  // If pk_type is string, set it up as such and remove autoincrement
  if (pk_type === 'string') {
    model.attributes.id.type = 'string';
    delete model.attributes.id.autoIncrement;
  }
  ...
  // Add wolfpack mock result methods
